...
# --- Vendored from diffusers 0.16.0 (scheduling_unipc_multistep.py) ---
# Adapted to run on the diffusers 0.11.1 base classes pinned by robomimic's
# Diffusion Policy (which still uses the old EMAModel API). Only the imports
# below and the `_compatibles` list were changed; the UniPC algorithm itself
# is byte-for-byte identical to upstream. See: https://arxiv.org/abs/2302.04867
...
class UniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    _compatibles = []  # KarrasDiffusionSchedulers enum not present in diffusers 0.11.1
    order = 1
    ...
    def convert_model_output(
        ...
        if self.predict_x0:
            ...
            # Static clipping of the predicted x0, mirroring DDPM/DDIM `clip_sample`.
            # Diffusion Policy actions live in [-1, 1]; this keeps UniPC apples-to-apples
            # with the DDPM baseline (which clips every step).
            if getattr(self.config, "clip_sample", False):
                r = self.config.clip_sample_range
                x0_pred = x0_pred.clamp(-r, r)
